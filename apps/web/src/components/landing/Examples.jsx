      ...
      <div className="shell">
        <Reveal className="flex flex-wrap items-end justify-between gap-6">
          <div>
            ...
            <h2 className="max-w-[20ch] font-display text-[clamp(1.9rem,4vw,2.9rem)] font-medium leading-[1.04] tracking-[-0.035em]">
              Launched with Launchpad.
            </h2>
          </div>
          <p className="max-w-[38ch] text-[14.5px] leading-relaxed text-ink-300">
        ...
        {!loading && !cards.length ? (
          <Reveal className="mt-12">
            <div className="rounded-card border border-dashed border-line-strong px-6 py-12 text-center">
              ...
              <p className="mx-auto mt-2 max-w-[46ch] text-[14px] leading-relaxed text-ink-300">
                The API is reachable but nothing has been published. Run <code className="rounded bg-white/5 px-1.5 py-0.5 font-mono text-[12.5px]">npm run seed</code> to
        ...
        <RevealGroup step={0.08} className="mt-12 grid gap-4 lg:grid-cols-3">
              ...
              >
                <div className={cx('relative overflow-hidden', index === 0 ? 'aspect-[16/9]' : 'aspect-[4/3]')}>
                  ...
                  <div className="absolute inset-x-0 bottom-0 p-5">
                    <p className="font-display text-[22px] leading-tight tracking-[-0.02em] text-white sm:text-[25px]">{item.name}</p>
                    <p className="mt-1 line-clamp-2 max-w-[52ch] text-[13.5px] leading-snug text-ink-200">{item.headline || item.tagline}</p>
                  </div>
                </div>
                <div className="mt-auto flex flex-wrap items-center gap-2 px-5 py-3.5">
                  <Tag mono>{item.type}</Tag>
                  <Tag>{item.sections} sections</Tag>
                  {item.publishedAt ? <Tag>{relativeTime(item.publishedAt)}</Tag> : null}
                  <span className="ml-auto flex items-center gap-1 font-mono text-[11.5px] text-ink-300 transition group-hover:text-white">
                    {item.displayUrl}
