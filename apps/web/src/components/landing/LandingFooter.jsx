    ...
    <footer className="relative border-t border-line py-14">
      <div className="shell">
        <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-[1.4fr_repeat(3,1fr)]">
          <div>
            <Logo />
            <p className="mt-4 max-w-[34ch] text-[13.5px] leading-relaxed text-ink-400">
            ...
            <div key={column.title}>
              ...
              <ul className="flex flex-col gap-2.5">
                {column.links.map((link) => (
                  <li key={link.label}>
                    {link.to ? (
                      <Link to={link.to} className="text-[13.5px] text-ink-300 transition hover:text-white">
                        {link.label}
                      </Link>
                    ) : (
                      <a href={link.href} className="text-[13.5px] text-ink-300 transition hover:text-white">
                        {link.label}
                      </a>
                    )}
                  </li>
        ...
        <div className="mt-12 flex flex-wrap items-center gap-x-6 gap-y-2 border-t border-line pt-6 text-[12.5px] text-ink-400">
          ...
          <span className="ml-auto font-mono text-[11.5px]">Built with Launchpad</span>
        </div>
      </div>
    </footer>
  );
