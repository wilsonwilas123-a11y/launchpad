    ...
    <section id="how" className="relative py-20 sm:py-28">
      <div className="shell">
        <Reveal className="max-w-[46ch]">
          ...
          <h2 className="font-display text-[clamp(1.9rem,4vw,2.9rem)] font-medium leading-[1.04] tracking-[-0.035em]">
            Seven steps, and none of them are design decisions.
          </h2>
        </Reveal>

        <RevealGroup step={0.055} className="relative mt-14 grid gap-x-10 gap-y-12 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <RevealItem key={step.key} className="relative">
              <article className="group flex flex-col">
                <div className="mb-5 flex items-center gap-3">
                  <span className="grid h-7 w-7 place-items-center rounded-full border border-line bg-ink-900 font-mono text-[11px] text-ink-100 transition group-hover:border-white/40 group-hover:text-white">
                    {index + 1}
                  </span>
                  <h3 className="font-display text-[21px] leading-none tracking-[-0.02em] text-white">{step.title}</h3>
                </div>
                <p className="text-[14.5px] leading-relaxed text-ink-200">{step.body}</p>
                {step.detail ? (
                  <p className="mt-3 border-l border-line pl-3 text-[13px] italic leading-relaxed text-ink-400">{step.detail}</p>
                ) : null}
              </article>
            </RevealItem>
          ))}
